...
def refit_solution(func, path, initial_guess):
    ...
    if npoints < len(initial_guess):
        return np.nan, [np.nan]*len(initial_guess)
    ...
    fit.migrad()
    y_pred = func(X, *fit.values)
    y_pred = np.where(np.abs(y_pred)<1e50, y_pred, np.sign(y_pred)*1e50)
    MSE_mvsr = mean_squared_error(y, y_pred)
    return MSE_mvsr, fit.values
...
def convert_string_to_func(SR_str, n_variables):
    ...
    # Make sure sqrt are not mistaken for parameters up to 10 sqrt intricated
    ...
    try:
        func = sp.lambdify(
            ...
            modules=[
                "numpy",
                {"exp": np.exp, "log": np.log, "sin": np.sin, "abs": np.abs, "cos": np.cos},
            ],
        )
    except:
        return sp.lambdify(
            ["X"] + ['A'],
            eval("A*X[0]", globals(), param_symbols),
            modules=["numpy",
                {"exp": np.exp, "log": np.log, "sin": np.sin, "abs": np.abs, "cos": np.cos},
            ],), "A*X1", {"A":0}
        ...
        for seed in range(nseeds):
            ...
            if not conversion[1]:
                ...
                if initial_guess != {}:
                    ...
                    results.iloc[seed] = [func_str, np.nan]
